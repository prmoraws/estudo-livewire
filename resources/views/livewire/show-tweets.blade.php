<div>
    ...
    @foreach ($tweets as $tweet)
        ...
        <p class="line-clamp-3">"{{ $tweet->content }}"</p><time
            class="font-serif text-sm">{{ $tweet->created_at }}</time><br>
        @if ($tweet->likes->count())
            <a href="#" wire:click.prevent="unlike({{ $tweet->id }})"
                class="no-underline hover:underline text-blue-600 text-sm">Descurtir</a>
        @else
            <a href="#" wire:click.prevent="like({{ $tweet->id }})"
                class="no-underline hover:underline text-blue-600 text-sm">Curtir</a>
        @endif
        <br>
    @endforeach
